...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size):
        if len(self.buffer) < batch_size:
            return list(range(len(self.buffer))), self.buffer, None
        ...
        return indices, [self.buffer[i] for i in indices], weights
    
    def update_priorities(self, indices, td_errors):
        for i, error in zip(indices, td_errors):
            self.priorities[i] = (error + self.epsilon) ** self.alpha
# DQN训练类（基于FCNN和Tensor）
class DQNTrainer:
    def __init__(self, state_size, action_size, hidden_layers=(128,), learning_rate=0.001, gamma=0.95,  epsilon=0.8, epsilon_decay=0.995, epsilon_min=0.01):
        ...
        self.memory = PrioritizedReplayBuffer(10000)  # 经验回放缓冲区
    def choose_action(self, state, training=True):
            ...
            if random.random() < self.epsilon and training:
                ...
                state_reshaped = state.reshape(1, -1)  # 转换为 (1, 4)
    ...
    def remember(self, state, action, reward, next_state, done):
        target = reward + self.gamma * np.max(self.target_net.forward(next_state).data)
        current = self.policy_net.forward(state).data.flatten()[action]
    ...
    def train_step(self, batch_size=32):
        ...
        _,samples,_ = self.memory.sample(batch_size)
...
def train_dqn(maze, start_pos, target_pos, episodes=2000, batch_size=32):
    ...
    state_size = maze_array.shape[0]*maze_array.shape[1]  # (x, y, target_x, target_y)
    action_size = 4  # 4种动作
    trainer = DQNTrainer(state_size, action_size, hidden_layers=(64,))
    ...
    for episode in range(episodes):
        ...
        state[agent.y*maze_array.shape[1]+agent.x] = 1
        total_reward = 0
        done = False
        visited = set()
        while not done and len(trainer.memory.buffer) < trainer.memory.capacity:
            prev_dist = np.sqrt((agent.x - target_pos[0])**2 + (agent.y - target_pos[1])**2)
            action = trainer.choose_action(state)
            reward = 0
            
            x, y = agent.move(action, maze_array)
            next_state = np.zeros(state_size)
            next_state[y*maze_array.shape[1]+x] = 1
            reward -= 1  # 基础移动奖励
            ...
            if maze_array[y, x] == -1:
                reward-=10
                    ...
                    # 距离奖励：如果更接近目标，给予正奖励
                new_dist = np.abs(target_pos[0]-x)+np.abs(target_pos[1]-y)
                reward += (prev_dist - new_dist)   # 距离变化放大10倍
...
# 测试流程
def test_dqn(trainer, maze, start_pos, target_pos,state_size):
    trainer.epsilon = 0
    maze_array = np.array(maze)
    agent = Agent(start_pos)
    state = np.zeros(state_size)
    ...
    while not done and len(path) < 100:
        ...
        path.append((x, y))
        state = np.zeros(state_size)
        state[y*maze_array.shape[1]+x] = 1
...
if __name__ == "__main__":
    ...
    # 测试
    print("\n开始测试...")
    path = test_dqn(trainer, maze, start_pos, target_pos,state_size)
    print("路径：", path)